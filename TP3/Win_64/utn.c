...
int getStringNew(char* pStr, char* msg, char*msgE)
{
    char bufferStr[20];
    printf("%s",msg);
    fgets(bufferStr,sizeof(bufferStr),stdin);
...
int getStringNumerosInt (char* pStr, char* msg, char* msgE,int reintentos)
{
    ...
    {
        if(!getStringNew(bufferStr,msg,msgE)&&(pStr!=NULL)&&(isNumberInt(bufferStr)))
        {
            strncpy(pStr,bufferStr,20);
            ret=0;
        }
...
int getStringLetras (char* pStr, char* msg, char* msgE,int reintentos)
{
    ...
    while(ret==-1 && reintentos>0)
    {
        if(!getStringNew(bufferStr,msg,msgE)&&(pStr!=NULL)&&(isLetter(bufferStr)))
        {
            strncpy(pStr,bufferStr,20);
            ret=0;
        }
